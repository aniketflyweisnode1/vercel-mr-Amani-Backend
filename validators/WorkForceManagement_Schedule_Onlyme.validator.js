...
const createScheduleOnlyMeSchema = Joi.object({
...
const updateScheduleOnlyMeSchema = Joi.object({
...
const getScheduleOnlyMeByIdSchema = Joi.object({
...
const baseListSchema = {
  ...
  sortBy: Joi.string().valid('name', 'created_at', 'updated_at', 'WorkForceManagement_Schedule_Onlyme_id').default('created_at'),
  sortOrder: Joi.string().valid('asc', 'desc').default('desc')
};

const getAllScheduleOnlyMeSchema = Joi.object(baseListSchema);
const getScheduleOnlyMeByAuthSchema = Joi.object(baseListSchema);

const getScheduleOnlyMeByTypeParamsSchema = Joi.object({
  type: Joi.string().trim().max(200).required()
});

const getScheduleOnlyMeByTypeQuerySchema = Joi.object({
  page: Joi.number().integer().min(1).default(1),
  limit: Joi.number().integer().min(1).max(100).default(10),
  search: Joi.string().trim().max(200).optional(),
  status: Joi.boolean().optional(),
