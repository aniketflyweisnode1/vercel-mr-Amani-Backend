...
const getRoleByIdSchema = Joi.object({
  id: Joi.string()
    ...
    .messages({
      ...
      'string.pattern.base': 'Please provide a valid role ID'
    })
});
...
const getAllRolesSchema = Joi.object({
  ...
  sortBy: Joi.string()
    .valid('name', 'created_at', 'updated_at', 'role_id')
    .default('created_at')
    .messages({
      'any.only': 'Sort by must be one of: name, created_at, updated_at, role_id'
    }),
