...
const createContactUs = asyncHandler(async (req, res) => {
  try {
    ...
    console.info('ContactUs created successfully', { contactUsId: contactUs._id, ContactUs_id: contactUs.ContactUs_id });
    sendSuccess(res, contactUs, 'ContactUs created successfully', 201);
  } catch (error) {
    console.error('Error creating contact us', { error: error.message, stack: error.stack });
    throw error;
  }
});
...
const getAllContactUs = asyncHandler(async (req, res) => {
  try {
    ...
    console.info('ContactUs list retrieved successfully', { total, page: parseInt(page), limit: parseInt(limit) });
    sendPaginated(res, contactUsList, pagination, 'ContactUs list retrieved successfully');
  } catch (error) {
    console.error('Error retrieving contact us list', { error: error.message, stack: error.stack });
    throw error;
  }
});
...
const getContactUsById = asyncHandler(async (req, res) => {
  try {
    ...
    console.info('ContactUs retrieved successfully', { contactUsId: contactUs._id });
    sendSuccess(res, contactUs, 'ContactUs retrieved successfully');
  } catch (error) {
    console.error('Error retrieving contact us', { error: error.message, contactUsId: req.params.id });
    throw error;
  }
});
...
const updateContactUs = asyncHandler(async (req, res) => {
  try {
    ...
    if (!contactUs) return sendNotFound(res, 'ContactUs not found');
    console.info('ContactUs updated successfully', { contactUsId: contactUs._id });
    sendSuccess(res, contactUs, 'ContactUs updated successfully');
  } catch (error) {
    console.error('Error updating contact us', { error: error.message, contactUsId: req.params.id });
    throw error;
  }
});
...
const deleteContactUs = asyncHandler(async (req, res) => {
  try {
    ...
    if (!contactUs) return sendNotFound(res, 'ContactUs not found');
    console.info('ContactUs deleted successfully', { contactUsId: contactUs._id });
    sendSuccess(res, contactUs, 'ContactUs deleted successfully');
  } catch (error) {
    console.error('Error deleting contact us', { error: error.message, contactUsId: req.params.id });
    throw error;
  }
});
